    ...
    <div className="space-y-4">
      ...
      {filteredCongregations.map(cong => {
        ...
        return (
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-1">
                <TextField label="Nome da Comum" value={cong.name} onChange={e => updateCongregation(cong.id, 'name', e.target.value.toUpperCase())} />
                <div className="grid grid-cols-3 gap-2">
                    {/* CORREÇÃO: Auto-Update do TXID ao digitar prefixo/sufixo */}
                    <TextField 
                        label="Prefixo (JB)" 
                        value={cong.shortPrefix} 
                        onChange={e => {
                            const val = e.target.value.toUpperCase();
                            updateCongregation(cong.id, 'shortPrefix', val);
                            updateCongregation(cong.id, 'txidBase', val + cong.ccbSuffix);
                        }} 
                    />
                    <TextField 
                        label="Sufixo (Cod)" 
                        ...
                        onChange={e => {
                            const val = e.target.value.toUpperCase();
                            updateCongregation(cong.id, 'ccbSuffix', val);
                            updateCongregation(cong.id, 'txidBase', cong.shortPrefix + val);
                        }} 
                    />
                    {/* CORREÇÃO: Formatação visual de centavos */}
                    <div>
                        <label className="block text-xs font-medium mb-1">Centavos</label>
                        <div className="relative">
                            <span className="absolute left-3 top-2.5 text-xs text-muted-foreground">R$ ***,</span>
                            <input 
                                type="number" 
                                className="w-full h-10 rounded-md border border-input bg-background pl-14 pr-3 text-sm"
                                value={cong.extraCents}
                                onChange={e => updateCongregation(cong.id, 'extraCents', Number(e.target.value))}
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="md:col-span-2 grid grid-cols-2 gap-3 border-t pt-2">
                    <div>
                        ...
                        {/* CORREÇÃO: Select clicável com valor garantido */}
                        <select 
                ...
                <TextField label="TXID Base (Automático)" value={cong.txidBase} onChange={e => updateCongregation(cong.id, 'txidBase', e.target.value.toUpperCase())} />
            </div>
          </AccordionItem>
      )})}
    </div>
